...
export async function GET(request: NextRequest) {
  return withErrorHandling(async () => {
    ...
    try {
      logger.info(`Fetching game props for event ${gameId} using hybrid cache`);
      
      // Use hybrid cache (Prisma + SDK)
      const response = await getGamePropsWithCache(gameId);
      const gameProps = response.data;
      
      logger.info(`Fetched ${gameProps.length} game props for event ${gameId} (source: ${response.source})`);
